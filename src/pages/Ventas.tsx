...
type Order = {
  ...
  items?: { productId?: string; id?: string; name: string; qty: number; price: number; isBeverage?: boolean }[];
};
...
export default function Ventas() {
  ...
  useEffect(() => {
    // Evito índice en DEV: solo where(orgId) y ordeno en cliente
    const qy = query(collection(db, "orders"), where("orgId", "==", ORG_ID));
  ...
  const canDelete = (o: Order) => o.status !== "delivered"; // evita borrar entregadas
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-3">
        ...
        return (
          <div key={o.id} className="bg-white border rounded-2xl p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                disabled={o.status !== "pending" || busy}
              ...
              <button
                disabled={!canDelete(o) || busy}
                className="px-3 py-1.5 rounded-lg border text-red-600 disabled:opacity-50"
                onClick={() => onDelete(o.id)}
                title={canDelete(o) ? "Eliminar definitivamente" : "No se puede eliminar entregada"}
              >
      ...
      {!list.length && (
        <div className="text-slate-500">No hay ventas {filter !== "all" ? (filter === "pending" ? "pendientes" : filter) : ""}.</div>
      )}
    </div>
  );
}
