...
export default function Clientes() {
  ...
  useEffect(() => {
    ...
    if (isStaff) {
      (async () => {
        try {
          const qy = fsQuery(collection(db, "customers"), where("orgId", "==", orgId), orderBy("displayName"));
    ...
    return (
      <div className="container-app p-6 pb-28 space-y-6">
        ...
        <div className="bg-white border rounded-2xl p-4 space-y-3">
          ...
          <div className="text-sm text-slate-600">Cr?f?'????T?f???�?,??"??f?'�?,?�?f��??s��??z�?f?'????T?f��??s�?,�?f?'?,�?f��?,?s?,�?f��?,?z?,�?f?'????T?f???�?,??"??f?'?,�?f��?,?s?,�?f????,�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�ditos disponibles</div>
  ...
  return (
    <div className="container-app p-6 pb-28 space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <input className="input w-64" placeholder="Buscar por nombre, email o ID" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
        ...
        <table className="min-w-full bg-white text-sm">
          <thead className="bg-slate-100 text-slate-700">
            <tr>
              ...
              <th className="text-left px-3 py-2">Cr?f?'????T?f???�?,??"??f?'�?,?�?f��??s��??z�?f?'????T?f��??s�?,�?f?'?,�?f��?,?s?,�?f��?,?z?,�?f?'????T?f???�?,??"??f?'?,�?f��?,?s?,�?f????,�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�ditos</th>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td className="px-3 py-6 text-center text-slate-500" colSpan={5}>Sin resultados.</td>
              </tr>
            )}
  ...
  return (
    <div className="flex gap-1">
      {Array.from({ length: total }).map((_, i) => (
        <div
          key={i}
          className={`w-4 h-4 rounded-full border ${i < value ? "bg-[var(--brand,#f97316)] border-[var(--brand,#f97316)]" : "bg-white"}`}
        />
      ))}
    </div>
  );
...
function Row({ c }: { c: Customer }) {
  ...
  const redeem = async () => {
    if (busy) return;
    if ((c.freeCredits || 0) <= 0) return alert("Sin cr?f?'????T?f???�?,??"??f?'�?,?�?f��??s��??z�?f?'????T?f��??s�?,�?f?'?,�?f��?,?s?,�?f��?,?z?,�?f?'????T?f???�?,??"??f?'?,�?f��?,?s?,�?f????,�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�ditos disponibles");
  ...
  return (
    <tr className="border-t">
      ...
      <td className="px-3 py-2">{c.email || "?f?'????T?f???�?,??"??f?'�?,?�?f��??s��??z�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�?f?'????T?f???�?,??"??f?'�?,?s?f??s?,�?f?'????T?f??s?,�?f?'?,�?f��??s�?.�?f??s?,�?f?'�?,?�?f??s?,�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�?f?'????T?f???�?,??"??f?'�?,?s?f??s?,�?f?'????T?f??s?,�?f?'?,�?f��?,?s?,�?f????,�?f?'�?,?s?f??s?,�?f?'????T?f��??s�?.�?f?'�?,?s?f??s?,�"}</td>
      <td className="px-3 py-2">
        <div className="flex items-center gap-2">
