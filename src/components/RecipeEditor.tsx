...
export default function RecipeEditor({ product }: { product: Product }) {
  ...
  useEffect(() => {
    (async () => {
      // Leer desde inventoryItems (consistente con Bodega/Compras)
    ...
    <div className="mt-3 p-3 rounded-xl border bg-slate-50">
      ...
      <div className="flex gap-2 mb-2">
        ...
        <input
          ...
          min={0}
        />
        <button onClick={add} className="px-3 py-2 rounded-lg bg-orange-500 text-white">
          Añadir
        </button>
      </div>
      ...
      <div className="mt-3 space-y-2">
        {entries.length === 0 && (
          <div className="text-slate-500 text-sm">Sin insumos aún.</div>
        )}
        {entries.map(([id, g]) => (
          <div
            key={id}
          ...
          >
            ...
            <button onClick={() => remove(id)} className="text-red-600 text-sm">
              Quitar
            </button>
          </div>
        ))}
