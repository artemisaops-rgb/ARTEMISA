...
export default function Apertura() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const id = `${ymd()}_${user.uid}`;
      await setDoc(doc(db, "openings", id), {
  ...
  return (
    <div className="container-app p-6 space-y-4">
      ...
      <div className="rounded-2xl border bg-white p-4 space-y-3">
        ...
        {["Limpieza del ?f?'????T?f???�?,??"??f?'�?,?s?f??s?,�rea","Puesto armado y ordenado","Equipos encendidos y probados","Insumos verificados (stock m?f?'????T?f???�?,??"??f?'�?,?s?f??s?,�nimo)"].map((txt)=>(
          <label key={txt} className="flex items-center gap-2 select-none">
            <input type="checkbox" className="accent-[var(--brand,#f97316)]"
              checked={checks.includes(txt)} onChange={()=>toggle(txt, checks, setChecks)} />
            <span>{txt}</span>
          </label>
        ))}
        ...
        {[
          {k:"foto_wpp_sent", label:"Envi?f?'????T?f???�?,??"??f?'�?,?s?f??s?,� la foto del inicio de caja al grupo"},
          {k:"foto_wpp_double", label:"Otra persona confirm?f?'????T?f???�?,??"??f?'�?,?s?f??s?,� (doble confirmaci?f?'????T?f???�?,??"??f?'�?,?s?f??s?,�n)"},
        ].map(({k,label})=>(
          <label key={k} className="flex items-center gap-2 select-none">
            <input type="checkbox" className="accent-[var(--brand,#f97316)]"
              checked={tasks.includes(k)} onChange={()=>toggle(k, tasks, setTasks)} />
            <span>{label}</span>
          </label>
        ))}
        ...
        <button disabled={!user || saving} onClick={save} className="btn">
          {saving ? "Guardando?f?'????T?f??s?,�?f?'?,�?f��??s�?.�?f??s?,�?f?'�?,?s?f??s?,�" : "Aperturar"}
        </button>
      </div>
    </div>
  );
